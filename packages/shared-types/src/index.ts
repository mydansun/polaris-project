...
/** One persisted round of clarification with its answers — returned as
 * part of SessionDetailResponse so the chat replay can re-render the
 * question card + user selections after a page reload. */
...
export type SessionEvent =
  ...
  | {
      // Mid-flight payload merge for an event still in started status.
      // Used to morph a single chat bubble through phases (e.g. the
      // discovery:references bubble flips searching → scoring → scored
      // without ever closing).  Frontend matches by external_id.
      session_id: string;
      run_id: string;
      kind: "event_payload_delta";
      event_kind: EventKind;
      external_id: string | null;
      payload: Record<string, unknown>;
    }
...
export type DeploymentSummary = {
  ...
  // MinIO public URL for the post-publish website screenshot.  Null
  // until the publish pipeline's screenshot step lands or a seed
  // backfills one.
...
export type ProjectResponse = {
  ...
  created_at: string;
  updated_at: string;
  // Server-side join from /api/projects.  Null when the project has
  // never been deployed.
  latest_deployment: DeploymentSummary | null;
  // Most-recent session's status (queued / running / completed /
  // failed / interrupted).  Lets the project drawer's status dot show
  // a red alert for projects that crashed in the agent flow before a
  // deployment row was ever created.
  latest_session_status: string | null;
  // True when this project has an active design_intents row.  False
  // when the project is fresh OR every prior discovery attempt was
  // interrupted/failed before persisting.  When false, the next user
  // message re-routes through ``discover_then_build`` (with all prior
  // user messages prepended) so an interrupted clarification can be
  // resumed by sending more text.
  has_active_design_intent: boolean;
  // Most-recent active design_intent's mood-board image URL, surfaced
  // here so HomePage cards can render a thumbnail in one round-trip.
