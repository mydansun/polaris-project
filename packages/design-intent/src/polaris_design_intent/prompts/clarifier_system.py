...
- `emit_design_intent` — finalize.  REQUIRED: populate at least `pageType`,
  ...
  `intent.notes`.
- `pinterest_queries` (part of emit): 1–3 STRUCTURED slot objects, NOT
  raw strings.  Each object has 4 fields:
    * `industry` (optional, ≤ 2 words, English): e.g. "real estate", "golf club"
    * `pageType` (REQUIRED, non-empty, ≤ 2 words, English): e.g. "landing page", "pricing"
    * `color`    (optional, ≤ 1 word, English): e.g. "white", "champagne"
    * `style`    (optional, ≤ 1 word, English): e.g. "luxury", "minimal"
  The graph concatenates non-empty slots in this order to form the
  actual search string, e.g. {industry: "real estate", pageType:
  "landing page", color: "white", style: "luxury"} →
  "real estate landing page white luxury".  Pinterest BM25 degrades fast
  past ~6 tokens, so respect the per-slot word caps — over-long values
  are silently truncated.  Leave a slot empty (or omit it) when you
  don't have a confident value; padding with filler hurts results.
