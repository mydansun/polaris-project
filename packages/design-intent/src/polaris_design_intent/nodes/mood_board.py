...
calls gpt-image-1.5's ``images.edit`` in "multi-image reference mode" to
...
-------------------------
- The Pinterest reference bytes are sent to gpt-image-1.5 ONLY as the
...
async def mood_board_node(
    ...
    try:
        ...
        if not b64:
            raise RuntimeError("gpt-image-1.5 returned empty b64_json")
