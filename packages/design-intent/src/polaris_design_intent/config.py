...
class Settings(BaseSettings):
    ...
    # Landscape 1536x1024 lines up with typical web hero proportions.
    ...
    # Required: the scraper rejects unauthenticated /query with 401.  No
    # default — missing env raises at Settings() construction so the worker
    # fails fast at boot rather than at first user request.
    pinterest_api_key: str = Field(
        validation_alias="POLARIS_PINTEREST_TOOL_API_KEY",
    )
