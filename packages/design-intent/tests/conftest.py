...
def settings() -> Settings:
    ...
    ``pinterest_api_key`` is required at construction time (no default), so
    we pass a stub via kwarg — that satisfies the field without depending
    on whatever the developer's local ``.env`` happens to have.
    """
    s = Settings(pinterest_api_key="test-pinterest-key")
