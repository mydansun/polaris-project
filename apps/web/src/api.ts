...
export async function deleteProject(projectId: string): Promise<void> {
  // DELETE returns 204 No Content; the generic ``request`` always calls
  // ``response.json()`` which would throw on an empty body, so we hit
  // fetch directly here (same pattern as ``getBrowserSession``).
...
export function getProjectCodexQuota(projectId: string): Promise<CodexQuotaResponse> {
  return request<CodexQuotaResponse>(`/projects/${projectId}/codex-quota`);
}
