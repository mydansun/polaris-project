...
function answerLabel(
  ...
  if (answer.override_text && answer.override_text.trim() !== "") {
    return answer.override_text.trim();
  }
  if (answer.selected_choice !== null) {
    const choice = question.choices.find((c) => c.id === answer.selected_choice);
...
export function ChatBubble({ message }: { message: ChatMessage }) {
  if (message.role === "user") {
    if (message.kind === "clarification") {
      const rows = message.questions
        .map((q) => ({ question: q, label: answerLabel(q, message.answers[q.id]) }))
            ...
            <div className="flex flex-col gap-2">
              {rows.map(({ question, label }) => (
                <div key={question.id} className="flex flex-col gap-0.5">
                  <span className="text-xs text-text-muted">{question.title}</span>
