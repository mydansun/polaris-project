  ...
  return useCallback(
    function applySessionEvent(raw: unknown) {
      ...
      setSessions((prev) => {
        ...
        if (event.kind === "event_payload_delta") {
          // Mid-flight payload merge — the event is still ``started``,
          // we just morph its inner state (e.g. discovery:references
          // flips searching → scoring with a refs gallery attached).
          // Match by external_id so the same bubble keeps morphing.
          if (event.external_id === null) return prev;
          ...
          const merged: EventResponse = {
            ...existing,
            payload_jsonb: {
              ...(existing.payload_jsonb as Record<string, unknown>),
              ...(event.payload as Record<string, unknown>),
            },
            updated_at: nowIso(),
          };
          const nextItems = entry.items.map((it, i) =>
            i === existingIdx ? merged : it,
          );
          return prev.map((e, i) =>
            i === index ? { ...entry, items: nextItems } : e,
          );
