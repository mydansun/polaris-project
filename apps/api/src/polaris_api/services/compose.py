...
def webview_short_hash(workspace_id: UUID) -> str:
    """12-char workspace identifier used in the webview hostname.

    Why a dedicated short id instead of reusing ``workspace_hash``
    (24 chars):  Theia v5 UUIDs are 36 characters, and DNS labels
    cap at 63 characters.  ``<36>-ide-<24>`` blows the limit
    (=65 chars).  Splitting the webview hostname out as
    ``wv-<36>-<12>`` keeps it at 52 chars with margin, AND lets us
    keep the existing ``ide-<24>`` IDE hostname / container names
    unchanged.  12 hex chars is 48 bits — collision-safe within any
    realistic single-tenant deployment.
    """
    return str(workspace_id).replace("-", "")[:12]


def webview_external_endpoint(workspace_id: UUID, domain: str) -> str:
    """Theia THEIA_WEBVIEW_EXTERNAL_ENDPOINT value for one workspace.

    Pattern: ``wv-{{uuid}}-<short>.<domain>`` where ``{{uuid}}`` is
    Theia's per-webview UUID placeholder and ``<short>`` is this
    workspace's short id.  Single subdomain label so the existing
    ``*.<domain>`` LE wildcard cert covers the URL.
    """
    return f"wv-{{{{uuid}}}}-{webview_short_hash(workspace_id)}.{domain}"


def _webview_host_regexp(workspace_id: UUID, domain: str) -> str:
    """Traefik v3 HostRegexp matcher for the webview hostname pattern
    above.  v3 takes raw Go regex; dots in the domain must be
    backslash-escaped so they don't match any character.  We anchor
    on the trailing ``-<short>.<domain>`` so the regex only catches
    THIS workspace's webviews — not other workspaces' (different
    short-hash) traffic.
    """
    short = webview_short_hash(workspace_id)
    escaped_domain = domain.replace(".", r"\.")
    return f"HostRegexp(`^wv-[a-z0-9-]+-{short}\\.{escaped_domain}$`)"
...
def render_workspace_runtime_compose(
    *,
    ...
    domain: str,
    shared_network: str = "polaris-shared",
    traefik_public_network: str = "traefik-public",
    project_id: UUID | None = None,
    workspace_token: str | None = None,
    api_url_for_workspace: str = "http://api:8000",
    ...
    workspace_environment = [
        ...
        # Theia per-webview hostname pattern.  Default is
        # ``{{uuid}}.webview.{{hostname}}`` — produces a 4-level
        # subdomain (<uuid>.webview.ide-<hash>.<domain>) that the
        # platform's ``*.<domain>`` LE wildcard cert can't cover (RFC
        # 6125: wildcards match exactly one label).  We override to a
        # SINGLE-label hostname ``wv-<uuid>-<short>.<domain>`` —
        # covered by the existing wildcard and short enough to fit
        # the DNS 63-char-per-label limit (~52 chars).  Traefik
        # routes via the HostRegexp label below.
        f"      THEIA_WEBVIEW_EXTERNAL_ENDPOINT: "
        f"{json.dumps(webview_external_endpoint(workspace_id, domain))}",
    ]
    ...
    lines = [
        ...
        *workspace_environment,
        ...
        # Parallel router for Theia per-webview origins.  Webview URL
        # pattern: ``wv-<webview-uuid>-<short>.<domain>``.  This
        # HostRegexp matches any UUID for THIS workspace's
        # ``<short>`` and routes to the same workspace container as
        # the bare IDE router above.  Single subdomain label, so
        # zero new certs needed.
        f"      traefik.http.routers.{ws_router}-webview.rule: "
        f"{json.dumps(_webview_host_regexp(workspace_id, domain))}",
        f"      traefik.http.routers.{ws_router}-webview.entrypoints: \"websecure\"",
        f"      traefik.http.routers.{ws_router}-webview.tls: \"true\"",
        f"      traefik.http.routers.{ws_router}-webview.service: {json.dumps(ws_router)}",
        "  chromium-vnc:",
    ...
    chrome_flags: list[str] = [
        ...
        # the workspace container via http://chromium-vnc:9223 (an nginx
        # proxy in the chromium-vnc container forwards 9223 → 9222), and `*` keeps
    ...
    #   - shared_network (polaris-shared): inter-project comms, pre-existing
...
async def start_workspace_runtime(
    *,
    ...
    try:
        ...
        # per-project `_default` network behind, which makes `up` fail
        # with "name already in use", "network already exists", or even
        # "no IPs left in pool" pointing at a network ID that no longer
        # exists (docker daemon caches per-project network bindings in
        # /var/lib/docker/network/files/local-kv.db; when polaris-shared
        # is recreated under a different ID, the cached pool is stale).
        # Recovery dance:
        #   1. compose down --remove-orphans (cleans most state)
        #   2. force-rm any containers labeled with this compose project
        #      (down won't touch them if they were created under a
        #      different project name)
        #   3. force-rm by container_name pattern (catches the few that
        #      use container_name override — workspace + chromium-vnc)
        #   4. retry up
        try:
            await run_compose(
                result.compose_path,
        ...
        hash_id = str(workspace_id).replace("-", "")[:24]
        for filter_arg in (
            f"label=com.docker.compose.project={result.project_name}",
            f"name=polaris-ws-{hash_id}",
            f"name=polaris-br-{hash_id}",
            f"name={result.project_name}-welcome",
        ):
            try:
                ls = await asyncio.create_subprocess_exec(
                    "docker", "ps", "-aq", "--filter", filter_arg,
                    stdout=asyncio.subprocess.PIPE,
                ...
                pass
        # Also drop the per-project _default network if it lingers — its
        # IPAM allocator can be in a weird state after a daemon-side
        # network recreate.
        try:
            net_rm = await asyncio.create_subprocess_exec(
                "docker", "network", "rm", f"{result.project_name}_default",
                stdout=asyncio.subprocess.DEVNULL,
                stderr=asyncio.subprocess.DEVNULL,
            )
            await net_rm.wait()
