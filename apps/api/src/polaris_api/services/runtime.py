...
_RUNTIME_LOCKS: dict[UUID, asyncio.Lock] = {}


def _runtime_lock(workspace_id: UUID) -> asyncio.Lock:
    """Process-wide per-workspace lock.

    Concurrent ensure / restart / stop calls for the same workspace
    would otherwise race on the per-project ``_default`` docker network:
    one call's compose-up creates it, a second call's compose-up fails
    on "name already in use" and runs the recovery dance (``compose
    down --remove-orphans`` + ``docker network rm``) which yanks the
    network out from under the first call mid-container-start, surfacing
    as ``failed to set up container networking: network ... not found``.

    The lock keeps these races inside the API process boundary; we
    don't have any cross-process coordination, but in dev the API runs
    as a single uvicorn worker so process-local is enough.
    """
    lock = _RUNTIME_LOCKS.get(workspace_id)
    if lock is None:
        lock = asyncio.Lock()
        _RUNTIME_LOCKS[workspace_id] = lock
    return lock
...
async def ensure_workspace_runtime(
    *,
    ...
    async with _runtime_lock(workspace.id):
        return await _ensure_workspace_runtime_locked(
            session=session, workspace=workspace, settings=settings
        )


async def _ensure_workspace_runtime_locked(
    *,
    session: AsyncSession,
    workspace: Workspace,
    settings: Settings,
    ...
    browser_session.context_metadata_jsonb = {
        **browser_session.context_metadata_jsonb,
        ...
        "cdp_endpoint": "http://chromium-vnc:9223",
