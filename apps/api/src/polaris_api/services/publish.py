...
def _sanitize_log(text: str | None, repo_path: Path | None) -> str | None:
    """Replace the host-side workspace path with the in-container alias
    ``/workspace`` before exposing log text to the user.

    The api process operates on bind-mounted host paths
    (``/home/<user>/.../.data/workspaces/<dev_uid>/<slug>``); subprocess
    stderr — and our own f-strings that include ``repo_path`` — would
    otherwise leak that absolute host path into the chat (Codex
    summaries reproduce it verbatim).  ``/workspace`` is the path the
    user sees inside the workspace container, so it stays intelligible
    without giving away platform layout.

    No-op when ``repo_path`` is None or the substring isn't present.
    """
    if text is None:
        return None
    if repo_path is None:
        return text
    needle = str(repo_path).rstrip("/")
    if not needle or needle not in text:
        return text
    return text.replace(needle, "/workspace")
...
async def run_publish(
    *,
    ...
    try:
        ...
        dep.build_log = _sanitize_log("".join(build_log), repo_path)
        await session.commit()
        logger.info("publish: deployment %s ready at %s", dep.id, domain)

        # Best-effort screenshot of the freshly-published site.  Failures
        # are swallowed by capture_and_record itself — a missing thumbnail
        # must not invalidate a successful publish.
        try:
            from polaris_api.services.publish_screenshot import capture_and_record

            await capture_and_record(
            ...
            logger.exception("publish: screenshot dispatcher crashed")
    except PublishError as exc:
        logger.warning("publish: deployment %s failed: %s", deployment_id, exc)
        dep.status = "failed"
        # ``repo_path`` may be unset if the failure happened before the
        # workspace lookup completed; the helper handles None gracefully.
        repo_for_sanitize = locals().get("repo_path")
        dep.error = _sanitize_log(str(exc), repo_for_sanitize)
        dep.build_log = _sanitize_log(
            "".join(build_log) if build_log else None, repo_for_sanitize
        )
        dep.smoke_log = _sanitize_log(
            "".join(smoke_log) if smoke_log else None, repo_for_sanitize
        )
        try:
            await session.commit()
        except Exception:
...
async def run_rollback(
    *,
    ...
    try:
        ...
        # Rollback path doesn't have its own repo_path in scope (it
        # works off the archive), but the log can still mention the
        # archive parent which we route through the same sanitizer.
        new_dep.build_log = _sanitize_log("".join(log), tmp.parent)
        await session.commit()
        await session.refresh(new_dep)

        # Best-effort screenshot — same pattern as forward publish.
        try:
            from polaris_api.services.publish_screenshot import capture_and_record

            await capture_and_record(
                deployment_id=new_dep.id,
                url=f"https://{new_dep.domain}/",
                db=session,
                settings=settings,
            )
        except Exception:  # noqa: BLE001
            logger.exception("rollback: screenshot dispatcher crashed")
        return new_dep
    finally:
