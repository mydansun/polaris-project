...
async def submit_clarification(
    ...
    # Persist answers + flip status before publishing so a chat replay
    # via GET /sessions/{id} reflects the user's selections.  Without
    # this, the row stayed at status='pending' / answers={} forever
    # while the answers only flowed through Redis to the worker.
    answers_payload = {
        qid: {
            "selected_choice": (ans or {}).get("selected_choice"),
            "override_text": (ans or {}).get("override_text"),
        }
        for qid, ans in body.answers.items()
    }
    clar_row = await db.scalar(
        select(Clarification).where(Clarification.request_id == body.request_id)
    )
    if clar_row is not None:
        clar_row.answers_jsonb = answers_payload
        clar_row.status = "answered"
        clar_row.answered_at = datetime.now(timezone.utc)
        await db.commit()
