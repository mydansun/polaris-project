...
async def _resolve_container_ip(
    ...
    """Look up the workspace container's IP **on the polaris-shared network**.

    Workspaces attach to three networks (per-project ``<compose>_default``,
    ``polaris-shared`` for platform DNS, ``traefik-public`` for ingress
    labels).  The worker container only lives on ``polaris-shared``, so we
    must pin to that network — otherwise ``docker inspect`` may return the
    per-project bridge IP (in 10.100.0.0/24) which is unreachable from the
    worker and the WS connect times out.  Retries for a short window so we
    can start concurrently with ``ensure_workspace_runtime`` on the API side.
    """
    ...
    while True:
        ...
        if proc.returncode == 0:
            ...
            if ip:
                return ip
            last_err = (
                "container has no IP on the polaris-shared network "
                "(workspace runtime may still be starting, or compose "
                "didn't attach it correctly)"
            )
        else:
