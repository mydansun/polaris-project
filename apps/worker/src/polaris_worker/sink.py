...
class DbEventSink(EventSink):
    ...
    async def emit_event_payload_delta(
        self,
        *,
        event_id: UUID,
        external_id: str | None = None,
        ...
        """Mid-flight payload update for an event still in ``started`` status.

        Used when a single chat bubble needs to morph through phases
        (e.g. ``discovery:references`` flips from a "searching"
        placeholder to a gallery + per-tile spinners) without ever
        being marked ``completed``.  The events row's payload_jsonb
        is merged with ``payload_delta`` so a late subscriber loading
        from the events table sees the accumulated state.  The
        pubsub envelope kind is ``event_payload_delta`` — the
        frontend hook merges it onto the still-open event without
        flipping it to terminal.
        """
        async with self._conn_lock:
            await self._conn.execute(
                "UPDATE events SET payload_jsonb = payload_jsonb || $1::jsonb, "
                "updated_at=now() WHERE id=$2",
                _jsonb_safe_dumps(payload_delta),
                event_id,
            )
            row = await self._conn.fetchrow(
                "SELECT kind, payload_jsonb FROM events WHERE id=$1",
                event_id,
            )
        if row is None:
            return
        full_payload = (
            json.loads(row["payload_jsonb"])
            if isinstance(row["payload_jsonb"], str)
            else (row["payload_jsonb"] or {})
        )
        await self._publish(
