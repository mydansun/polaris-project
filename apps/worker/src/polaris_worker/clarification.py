...
async def wait_for_answers(
    *,
    ...
    When ``conn``/``conn_lock``/``project_id`` are provided, a
    ``Clarification`` row is persisted at ``status='pending'`` *before*
    the SSE fires — the API's ``/clarify/response`` route flips it to
    ``'answered'`` with the user's selections so a chat replay via
    ``GET /sessions/{id}`` can re-render the round.
    ...
    if conn is not None and conn_lock is not None and project_id is not None:
        row_id = uuid4()
        async with conn_lock:
            await conn.execute(
                """
                INSERT INTO clarifications (
                    id, request_id, project_id, session_id, run_id,
                    agent_kind, status, questions_jsonb, answers_jsonb
                )
...
def build_design_intent_user_input_fn(
    *,
    ...
    When ``conn``/``conn_lock``/``project_id`` are supplied, the underlying
    :func:`wait_for_answers` persists a Clarification row so the chat
    replay can re-render the round.
    """
