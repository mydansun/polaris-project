...
async def _collect_resumed_user_message(
    ...
    *,
    ...
    """If discovery is being re-run (current call is
    ``mode=discover_then_build`` but the project has NO active
    design_intent), gather every prior session's ``user_message`` for
    the same project and concat with the current one — clarifier sees
    the full intent across user retries.

    Returns the input unchanged when:
      * an active design_intent already exists (this is a real
        re-discover, not a resume of a never-completed one), or
      * there are no prior sessions, or
      * any prior message is identical to the current one (caller
        likely just retried the same string — no value in stuttering).

    No dedup of repeated CONTENT inside the merged string;
    duplicate-detection is best-effort and the LLM tolerates a bit
    of repetition.
    """
    # If discovery already produced an active intent, don't merge —
    # the worker side ``seed_intent`` mechanism already carries the
    # prior compiled brief into the new graph run.
    has_active = await conn.fetchval(
        "SELECT 1 FROM design_intents "
    ...
    rows = await conn.fetch(
        "SELECT user_message FROM sessions "
    ...
    prior = [r["user_message"] for r in rows if r["user_message"]]
    if not prior:
        return current_user_message
    if any(m == current_user_message for m in prior):
        return current_user_message
    return "\n\n".join([*prior, current_user_message])
...
async def process_session_job(
    ...
    # If this session is a re-run of discovery (mode=discover_then_build
    # but no prior session for the same project produced an active
    # design_intent), prepend prior interrupted/failed runs'
    # user_messages so the clarifier sees the full intent the user
    # has been trying to express across multiple attempts.  We do
    # this server-side (NOT in the frontend) so the chat bubble
    # keeps showing exactly what the user typed — only the agent
    # input gets the merged context.
    if mode == "discover_then_build":
        merged = await _collect_resumed_user_message(
            conn,
            project_id=project_id,
            current_session_id=session_id,
            current_user_message=user_message,
        )
        if merged != user_message:
            n_prior = merged.count("\n\n")
            log.info(
                "discovery resume: prepended %d prior user_message(s) (%d → %d chars)",
                n_prior,
                len(user_message),
                len(merged),
            )
