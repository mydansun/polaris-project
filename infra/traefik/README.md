...
- **`polaris-shared`** — created by `compose.dev.yaml`, shared with
  per-workspace compose projects. Traefik joins it so it can reach
  workspace/chromium container names for dev routing.
- **`traefik-public`** — created by this compose file. Published user
  projects join it as `external: true` so their traefik labels land
  on a network traefik watches.

Dev containers join `polaris-shared`; published containers join
`traefik-public`.
