...
Talks to the platform API at $POLARIS_API_URL (default http://api:8000 — resolves on
the shared `polaris-shared` docker network that workspace + api containers both join).
Legacy host-mode deployments override to http://host.docker.internal:8000.
...
def api_base() -> str:
    return os.environ.get("POLARIS_API_URL", "http://api:8000").rstrip("/")
